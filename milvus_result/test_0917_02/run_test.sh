#!/usr/bin/env bash
# Runs the bounded Plain Query vector-payload Chunk-size sweep.
...
COMPOSE_PROJECT="qv-query-chunks-$(printf '%s' "$RUN_ID" | tr '[:upper:]' '[:lower:]' | tr -cd '[:alnum:]-')"
...
PROFILE_CHUNK_SIZES=(256 1024 8192)
WARMUP_OPERATIONS=5
MIN_OPERATIONS=30
MIN_DURATION_SECONDS=60
REPETITIONS=2
# Fixed fan-in: N16 primary first, N32 confirmation.
TOPOLOGIES=(16 32)
...
start_role() {
    ...
    local log_level=${5:-info} query_streaming=${6:-unused} chunk_size=${7:-1024}

    log "Starting $name, query streaming=$query_streaming chunk=$chunk_size"
    server_bash \
        "$name" "$role" "$metrics_port" "$rpc_port" "$log_level" \
        "$query_streaming" "$chunk_size" "$SERVER_RUN_DIR" "$SERVER_REPO" "$SERVER_ENV" <<'REMOTE'
set -Eeuo pipefail
name=$1
role=$2
metrics_port=$3
...
(
    ...
    if [[ "$role" == "querynode" ]]; then
        ...
        export MILVUS_CONF_PROXYQUERYVIEWQUERYSTREAMCHUNKSIZE="$chunk_size"
    fi
...
# start_proxy restarts Proxy in the requested mode with the given Chunk size.
start_proxy() {
    local mode=$1 chunk_size=${2:-1024} log_level=${3:-info}
    local streaming=false
    [[ "$mode" == streaming ]] && streaming=true
    stop_pid_file "$SERVER_RUN_DIR/pids/proxy.pid"
    start_role proxy proxy "$PROXY_METRICS_PORT" 0 "$log_level" "$streaming" "$chunk_size"
...
run_correctness_gate() {
    ...
    start_proxy batch 1024 info
    run_client_action "$case_name-batch-verify" \
        "verify --host 10.15.9.42 --port '$MILVUS_PORT' \
        --collection '$COLLECTION_NAME' --mode batch --limit '$QUERY_LIMIT' \
        --output '$remote_case_dir/batch-verify.json'"
    copy_from_client "$remote_case_dir/batch-verify.json" "$case_dir/batch-verify.json"

    for chunk in "${CHUNK_SIZES[@]}"; do
        start_proxy streaming "$chunk" info
        run_client_action "$case_name-chunk-$chunk-verify" \
            "verify --host 10.15.9.42 --port '$MILVUS_PORT' \
            --collection '$COLLECTION_NAME' --mode streaming --limit '$QUERY_LIMIT' \
            --chunk-size '$chunk' --output '$remote_case_dir/chunk-$chunk-verify.json'"
        copy_from_client "$remote_case_dir/chunk-$chunk-verify.json" \
            "$case_dir/chunk-$chunk-verify.json"
    done
...
if batch["count"] != 8192:
    raise SystemExit(f"batch returned {batch['count']} rows")
if batch["logicalFieldBytes"] != 8192 * (8 + 768 * 4):
    raise SystemExit("batch logical field-byte count is unexpected")
for path in sorted(root.glob("chunk-*-verify.json")):
    streaming = json.load(open(path, encoding="utf-8"))
    if streaming["count"] != 8192:
        raise SystemExit(f"{path.name} returned {streaming['count']} rows")
    if streaming["logicalFieldBytes"] != 8192 * (8 + 768 * 4):
        raise SystemExit(f"{path.name} logical field-byte count is unexpected")
    if batch["ids"] != streaming["ids"]:
        raise SystemExit(f"{path.name} ordered IDs differ from Batch")
    if batch["sha256"] != streaming["sha256"]:
        raise SystemExit(f"{path.name} ID/vector hash differs from Batch")
json.dump({
    "status": "PASS",
    "count": batch["count"],
    "logicalFieldBytes": batch["logicalFieldBytes"],
    "sha256": batch["sha256"],
    "chunkSizes": sorted(int(path.stem.split("-")[1]) for path in root.glob("chunk-*-verify.json")),
...
run_interval() {
    local case_name=$1 repetition=$2 mode=$3 chunk=${4:-1024}
    local label="$case_name/rep$repetition/$mode-chunk$chunk"
    ...
    run_client_action "$case_name-rep$repetition-$mode" \
        ...
        --repetition '$repetition' --limit '$QUERY_LIMIT' --chunk-size '$chunk' \
...
run_cpu_profile() {
    local case_name=$1 mode=$2 chunk=${3:-1024}
    local label="$case_name/profiles/$mode-$chunk"
    local local_dir="$RUN_DIR/$label"
    local remote_client_dir="$CLIENT_RUN_DIR/$label"
    ...
    local profile_pid=$!
    sleep 1
    run_client_action "$case_name-profile-$mode-$chunk" \
        "benchmark --host 10.15.9.42 --port '$MILVUS_PORT' \
        --collection '$COLLECTION_NAME' --case '$case_name-profile' --mode '$mode' \
        --limit '$QUERY_LIMIT' --chunk-size '$chunk' --warmup-operations 2 \
...
run_topology() {
    ...
    log "Running $case_name Batch rep 1 (pre-sweep baseline)"
    run_interval "$case_name" 1 batch 1024
    run_correctness_gate "$case_name" "$case_dir" "$remote_case_dir"

    for chunk in "${CHUNK_SIZES[@]}"; do
        for ((repetition = 1; repetition <= REPETITIONS; repetition++)); do
            log "Running $case_name chunk $chunk rep $repetition streaming"
            run_interval "$case_name" "$repetition" streaming "$chunk"
        done
    done

    log "Running $case_name Batch rep 2 (post-sweep baseline)"
    run_interval "$case_name" 2 batch 1024

    run_cpu_profile "$case_name" batch 1024
    for profile_chunk in "${PROFILE_CHUNK_SIZES[@]}"; do
        run_cpu_profile "$case_name" streaming "$profile_chunk"
    done
...
# 2 cases x (2 Batch + 6 chunk sizes x 2 Streaming reps) = 28 timed intervals.
if len(rows) != 28:
    raise SystemExit(f"found {len(rows)} timed intervals, expected 28")
...
for case_dir in case_dirs:
    ...
    batch_rows = [row for row in rows
                  if row["case"] == case_dir.name and row["mode"] == "batch"]
    batch_qps = statistics.median(row["qps"] for row in batch_rows)
    batch_p95 = statistics.median(row["p95_ms"] for row in batch_rows)
    batch_rss = statistics.median(row["peak_proxy_rss_mib"] for row in batch_rows)
    batch_cpu = statistics.median(row["median_proxy_cpu_percent"] for row in batch_rows)
    for chunk in correctness["chunkSizes"]:
        streaming_rows = [row for row in rows
                          if row["case"] == case_dir.name and row["mode"] == "streaming"
                          and row["chunkSize"] == chunk]
        streaming_qps = statistics.median(row["qps"] for row in streaming_rows)
        streaming_p95 = statistics.median(row["p95_ms"] for row in streaming_rows)
        summary.append({
            "case": case_dir.name,
            "chunkSize": chunk,
            ...
            "batchMedianPeakProxyRssMiB": batch_rss,
            "streamingMedianPeakProxyRssMiB": statistics.median(row["peak_proxy_rss_mib"] for row in streaming_rows),
            "batchMedianProxyCpuPercent": batch_cpu,
            "streamingMedianProxyCpuPercent": statistics.median(row["median_proxy_cpu_percent"] for row in streaming_rows),
        })
if len(hashes) != 1:
...
main() {
    ...
    build_summary
    log "All Chunk-size Batch-versus-Streaming Query intervals passed"
}
