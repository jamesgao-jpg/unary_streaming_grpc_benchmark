#!/usr/bin/env bash
# Runs the bounded Plain Query vector-payload concurrency sweep.
...
COMPOSE_PROJECT="qv-query-conc-$(printf '%s' "$RUN_ID" | tr '[:upper:]' '[:lower:]' | tr -cd '[:alnum:]-')"
...
PROFILE_CONCURRENCIES=(1 32)
WARMUP_OPERATIONS=5
MIN_OPERATIONS=30
MIN_DURATION_SECONDS=60
REPETITIONS=2
# Single fixed fan-in topology; concurrency is the swept variable.
TOPOLOGIES=(16)
...
# benchmark measures concurrent Query latency while validating row counts.
def benchmark(args):
    connect(args)
    collection = Collection(args.collection)
    ...
    def client_loop():
        ...
        while True:
            with lock:
                finished = len(latencies) >= args.min_operations and \
                    time.monotonic() - started >= args.min_duration_seconds
            if finished:
                return
    ...
    threads = [threading.Thread(target=client_loop) for _ in range(args.concurrency)]
...
run_correctness_gate() {
    ...
    for mode in batch streaming; do
        start_proxy "$mode" "$CHUNK_SIZE" info
        run_client_action "$case_name-$mode-verify" \
            "verify --host 10.15.9.42 --port '$MILVUS_PORT' \
            --collection '$COLLECTION_NAME' --mode '$mode' --limit '$QUERY_LIMIT' \
            --chunk-size '$CHUNK_SIZE' --output '$remote_case_dir/$mode-verify.json'"
...
# run_interval executes one timed mode interval at a concurrency level.
run_interval() {
    local case_name=$1 repetition=$2 mode=$3 concurrency=${4:-1}
    local label="$case_name/conc$concurrency/rep$repetition/$mode"
    ...
    run_client_action "$case_name-conc$concurrency-rep$repetition-$mode" \
        "benchmark --host 10.15.9.42 --port '$MILVUS_PORT' \
        --collection '$COLLECTION_NAME' --case '$case_name' --mode '$mode' \
        --repetition '$repetition' --limit '$QUERY_LIMIT' --chunk-size '$CHUNK_SIZE' \
...
run_cpu_profile() {
    local case_name=$1 mode=$2 concurrency=${3:-1}
    local label="$case_name/profiles/$mode-conc$concurrency"
    local local_dir="$RUN_DIR/$label"
    local remote_client_dir="$CLIENT_RUN_DIR/$label"
    ...
    local profile_pid=$!
    sleep 1
    run_client_action "$case_name-profile-$mode-conc$concurrency" \
        "benchmark --host 10.15.9.42 --port '$MILVUS_PORT' \
        --collection '$COLLECTION_NAME' --case '$case_name-profile' --mode '$mode' \
        --limit '$QUERY_LIMIT' --chunk-size '$CHUNK_SIZE' \
        --concurrency '$concurrency' --warmup-operations 2 \
...
# run_topology loads the collection, qualifies it, and measures every
# concurrency level with balanced Batch/Streaming order.
run_topology() {
    local count=$1
    local case_name="FANIN-N$count"
    local case_dir="$RUN_DIR/$case_name"
    local remote_case_dir="$CLIENT_RUN_DIR/$case_name"
    local concurrency repetition
    local -a modes
    ...
    for concurrency in "${CONCURRENCIES[@]}"; do
        for ((repetition = 1; repetition <= REPETITIONS; repetition++)); do
            ...
            for mode in "${modes[@]}"; do
                log "Running $case_name concurrency $concurrency rep $repetition mode $mode"
                run_interval "$case_name" "$repetition" "$mode" "$concurrency"
            done
        done
    done

    for profile_concurrency in "${PROFILE_CONCURRENCIES[@]}"; do
        run_cpu_profile "$case_name" batch "$profile_concurrency"
        run_cpu_profile "$case_name" streaming "$profile_concurrency"
    done
...
# 1 case x 6 concurrency levels x 2 reps x 2 modes = 24 timed intervals.
if len(rows) != 24:
...
for case_dir in case_dirs:
    ...
    for concurrency in sorted({row["concurrency"] for row in rows
        ...
        summary.append({
            "case": case_dir.name,
            "concurrency": concurrency,
            "chunkSize": correctness["chunkSize"],
            ...
            "batchMedianPeakProxyRssMiB": statistics.median(row["peak_proxy_rss_mib"] for row in batch_rows),
            "streamingMedianPeakProxyRssMiB": statistics.median(row["peak_proxy_rss_mib"] for row in streaming_rows),
            "batchMedianProxyCpuPercent": statistics.median(row["median_proxy_cpu_percent"] for row in batch_rows),
            "streamingMedianProxyCpuPercent": statistics.median(row["median_proxy_cpu_percent"] for row in streaming_rows),
        })
if len(hashes) != 1:
    raise SystemExit("result hash differs across concurrency levels")
...
main() {
    ...
    start_base_roles
    start_proxy batch "$CHUNK_SIZE" info
    ...
    build_summary
    log "All concurrency Batch-versus-Streaming Query intervals passed"
}
